...
<body>
  ...
  enableCaseInsensitiveFiltering: true
    ...
    <?php
      ...
      if(!empty($client))
      {
        ...
        while($row = mysqli_fetch_array($result))
        {
           ...
           elseif($row['Field']=="click" ) {
                echo '<th name='.$row['Field'].'>Landing Page URL</th>';
           }
           elseif($row['Field']=="impression" ) {
                echo '<th name='.$row['Field'].'>Impression tracker</th>';
           }
           elseif($row['Field']=="bg" ) {
                echo '<th name='.$row['Field'].'>Background Image</th>';
           }
           elseif($row['Field']=="blue" ) {
                echo '<th name='.$row['Field'].'>Blue Panel</th>';
           }
           elseif($row['Field']=="title" ) {
                echo '<th name='.$row['Field'].'>Movie Title</th>';
           }
           elseif($row['Field']=="logo" ) {
                echo '<th name='.$row['Field'].'>Brand Logo</th>';
           }
           elseif($row['Field']=="cta" ) {
                echo '<th name='.$row['Field'].'>CTA</th>';
           }
           elseif($row['Field']=="copy" ) {
                echo '<th name='.$row['Field'].'>Copy</th>';
           }
           else{
            ...
            while($row = mysqli_fetch_array($result))
            {
                ...
                for ($i=2;$i<=$rowcount-1;$i++)
                    { 
                        ...
                        else if($name[$i]=="click"){
                            echo '<td><input placeholder="Add Landing Url/DCM click tracker" type="text" name="'.$name[$i].$row['id'].'"value="'.$row[$i].'"></td> ';
                        }
                        else if($name[$i]=="impression"){
                            echo '<td><input placeholder="Add DCM Impression Tracker" type="text" name="'.$name[$i].$row['id'].'"value="'.$row[$i].'"></td> ';
                        }
    ...
    ?> 
    <button type="button" onclick="addcolumn()">Add New Assets</button> 
     <button type="submit" name="savec" value="savec">Save New Assets</button>
